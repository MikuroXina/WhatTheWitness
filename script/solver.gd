...
var is_start: Array
var is_end: Array
var is_region_enabled: Array
var solution_state: DiscreteSolutionState
...
func to_solution_line():
	...
	var solution_line = SolutionLine.new()
	var stacking_solution_state = DiscreteSolutionState.new()
				...
				while ok:
					...
					for v2 in vertice_neighbors[v]:
						if sugar_solution[is_solution[v2]] == way and not (v2 in visited_vertices):
							...
							ok = true
							break
		stacking_solution_state.vertices.append(way_vertices)
	solution_line.state_stack.append(stacking_solution_state)
...
func ensure_segment():
	...
	for v_pair in region_edge_list:
		s.ensure(s.or_(s.or_(s.eq(is_region_enabled[v_pair[0]], -1), s.eq(is_region_enabled[v_pair[1]], -1)),
			s.and_(s.eq(is_region_enabled[v_pair[0]], is_region_enabled[v_pair[1]]),
			s.iff(is_tetris_covered[v_pair[0]], is_tetris_covered[v_pair[1]]))
		))
	for region_id in range(n_max_regions):
		s.ensure(s.graph_vertex_connected(s.eq(is_region_enabled, region_id), region_edge_list))
		...
		for v2 in range(n_vertices):
			...
			s.ensure(
				s.imp(s.and_(s.eq(is_decorator[v], rule_ids['square']),
					...
					s.neq(is_region_enabled[v], is_region_enabled[v2]))
			)
	...
	for v in range(n_vertices):
		...
		for covering in puzzle.vertices[v].decorator.covering:
			...
			for f in covering:
				...
				# var region_cond = s.eq(is_region_enabled[v], is_region_enabled[vf])
	...
	for ref_v in range(n_vertices):
		...
		for f in range(len(puzzle.facets)):
			...
			for b_v_pair in coverings_facets_pos[f]:
				pos_cond.append(s.and_(b_v_pair[0], s.eq(is_region_enabled[ref_v], is_region_enabled[b_v_pair[1]])))
			for b_v_pair in coverings_facets_neg[f]:
				neg_cond.append(s.and_(b_v_pair[0], s.eq(is_region_enabled[ref_v], is_region_enabled[b_v_pair[1]])))
			s.ensure(
				s.eq(
					s.sub(
						s.count_true(pos_cond),
						s.count_true(neg_cond),
					),
					s.if_(
						s.and_(
							s.eq(
								is_region_enabled[center_v],
								is_region_enabled[ref_v],
							),
							is_tetris_covered[center_v],
						),
						1,
						0,
					),
...
func ensure_minesweeper():
	...
	for v in range(n_vertices):
		...
		for v2 in neighbor_facet_vertex_ids:
			count.append(s.neq(is_region_enabled[v], is_region_enabled[v2]))
		# print(s.eq(s.count_true(count), puzzle.vertices[v].decorator.count))
		s.ensure(s.eq(s.count_true(count), puzzle.vertices[v].decorator.count))
	...
	for v in range(n_vertices):
		...
		for v2 in range(n_vertices):
			...
			if puzzle.vertices[v2].decorator.color != color:
				continue
			choices.append(s.eq(is_region_enabled[v], is_region_enabled[v2]))
...
func ensure_circle_arrow():
	...
	for v_pair in edges_direction:
		...
		s.ensure(
			s.imp(
				s.eq(
					is_solution[v_pair[0]],
					is_solution[v_pair[1]],
				),
				s.xor(
					edges_direction[v_pair],
					edges_direction[[v_pair[1], v_pair[0]]],
				),
			),
		)
