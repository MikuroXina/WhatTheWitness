...
func __finish_solution(puzzle: Puzzle, delta: Vector2):
	...
	var new_state_with_limit = state_stack[-1].transit(puzzle, vertex_id)
...
func save_to_string(puzzle: Puzzle) -> String:
	...
	for state_way in state.vertices:
		...
		line_result.append( ','.join(way_result))
	var line_string = '|'.join(line_result)
	var event_property_result = PackedStringArray()
	...
	var event_string = '|'.join(event_property_result)
