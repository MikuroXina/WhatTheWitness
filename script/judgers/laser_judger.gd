...
func laser_pass_point(validator: Validation.Validator, pos: Vector2, color: Color) -> bool:
	...
	for i in range(len(puzzle.decorators)):
			...
			for k in range(len(lasers)):
				...
				for j in range(len(laser) - 1):
					if Geometry2D.segment_intersects_circle(laser[j], laser[j + 1], pos, TARGET_SIZE) >= 0:
						...
						break
		if passed_laser_colors.is_empty():
			return false
		...
		return (
			abs(color_comp[0] - color.r) <= 1e-2 and
			abs(color_comp[1] - color.g) <= 1e-2 and
			abs(color_comp[2] - color.b) <= 1e-2
		)
	return false
