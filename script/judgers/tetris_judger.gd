...
func calc_triangle_area(v1: Vector2, v2: Vector2, v3: Vector2) -> float:
	return det(v2 - v1, v3 - v1) / 2

func calc_area(vertex_array: Array) -> float:
	var result = 0.0
	for i in range(2, len(vertex_array)):
		result += calc_triangle_area(vertex_array[0], vertex_array[i - 1], vertex_array[i])
	return abs(result)

func calc_area_vertices(vertex_array: Array) -> float:
	var result = 0.0
	for i in range(3, len(vertex_array)):
		result += calc_triangle_area(vertex_array[0], vertex_array[i - 1], vertex_array[i])
	return abs(result)

func judge_region_tetris_implementation(validator, region: Validation.Region, require_errors: bool) -> bool:
	var cell_count = 0
	var area_sum = 0.0
	var has_multi = false
	...
	if has_weak and !judge_csp(validator, region, true, true):
		if (require_errors):
			for decorator_id in region.decorator_dict['tetris']:
				var response = validator.decorator_responses[decorator_id]
				if (response.decorator.is_weak):
